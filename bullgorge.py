...
class Frontend(Frame):
    ...
    def createWidgets(self):
        ...
        for child in self.valuesframe.winfo_children():
            child.grid_configure(padx=3, pady=3)
...
class Server():
    ...
    def check_paths(self):
        # check if we can find the hldsupdatetool executable
        open(os.path.join(self.hlds_path, self.hlds_exe)).close()
        # check if we can find the ns2 server executable
        open(os.path.join(self.server_path, self.server_exe)).close()
    ...
    def guard_server(self):
        ...
        while self.runserver:
            ...
            while p.poll() == None:
                time.sleep(1)
                if self.gui:
                    # if the console has been closed, we need to kill ourselves
                    if not self.cls.is_alive():
...
if __name__ == '__main__':
    ...
    try:
        ...
        # print the error, then go to clean exit
        print(e)
        ...
        if srv.server != None and srv.server.poll():
            # make sure the server dies if we do
            srv.server.terminate()
            # also, don't quit before the server is dead
            srv.server.wait()
        if srv.gui:
            # we need to wait until the console is closed
            srv.cls.join()
